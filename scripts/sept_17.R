...
# Could we try to learn something about that wreck with 52 people in it? 
# Let's use the persons df

# Do we have any missing keys? NO MISSING KEYS
...
## one-to-many: unique on the left, repeats on the right <- We will use this one for this data
...
## left - joins matches, does not drop the lack of matches
...
## One row represents one crash-person record.
...
persons_left_borough <- crashes_core %>% 
  left_join(persons_core, join_by(COLLISION_ID))
